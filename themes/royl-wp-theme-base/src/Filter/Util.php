...
use Royl\WpThemeBase\Util\Configure;
...
class Util
{
     ...
     */
    public static function getQueryVar(string $var): mixed
    {
    ...
    /**
     * Get an array of all the defined filters and the filter template mappings for specific filter set
     * @param string $set the filter set to get defined filters for
     * @return array
     */
    private static function getDefinedFilterData(string $set): array
    {
        return [
            'filters' => Configure::read('filters.filters'),
            'filterlist' => Configure::read('filters.filter_groups.' . $set)
        ];
    }
     ...
     */
    public static function renderFilterForm(string $set, string $partial = 'filter-bar'): void
    {
        /**
     ...
     * @return WP_Query
     */
    public static function getFilterQuery($set): WP_Query
    {
